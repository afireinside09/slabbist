...
|---|---|---|---|
| Role-based buy-price visibility "enforced in the database" (owner sees comp/cost/margin, associate sees buy only) | hero capability; feature-row 4th tab + MarginRulesPanel; features COUNTER + BACK_OFFICE; for-shops point; changelog v0.8.0 | NOT FOUND in app (a `StoreRole` enum exists at `ios/slabbist/slabbist/Core/Models/StoreMember.swift:4-5` but is never used to gate buy-price/comp/margin visibility in any view — model/repo/DTO plumbing only, zero role-branching in Features/) | Replaced with the real margin ladder; multi-tenant store scoping kept (that IS real via RLS) |
...
| Print/email offer sheets | features COUNTER; for-shops; changelog v0.9.0 | NOT FOUND (only incidental hit: a "printed frame edge" comment) | "Offer sheet, ready to present" (on-screen lot total + line items + payment method/ref + mark paid) |
| On-device signature capture / signed PDF | features COUNTER; for-shops; for-vendors; changelog v0.9.0 | NOT FOUND (only incidental hit: SF Symbol `square.and.arrow.up`) | Removed |
