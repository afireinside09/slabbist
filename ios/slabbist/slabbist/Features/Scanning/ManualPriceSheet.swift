...
struct ManualPriceSheet: View {
    ...
    init(initialCents: Int64?, onSubmit: @escaping (Int64?) throws -> Void) {
        ...
        // Pre-fill with the existing offer (if any) in dollars.cents form so
            ...
            VStack(alignment: .leading, spacing: Spacing.xxl) {
                ...
                if let error {
                    Text(error)
                        ...
                        .accessibilityIdentifier("manual-price-error")
                }
                Spacer()
                if initialCents != nil {
                    Button(action: clear) {
                        Text("Clear manual price")
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("manual-price-clear")
                }
                PrimaryGoldButton(
                    title: initialCents == nil ? "Save price" : "Update price",
                    isEnabled: !trimmed.isEmpty
                ) {
                    submit()
                }
                .accessibilityIdentifier("manual-price-save")
            }
        ...
        VStack(alignment: .leading, spacing: Spacing.s) {
            ...
            SlabCard {
                HStack(spacing: Spacing.m) {
                    ...
                    .tint(AppColor.gold)
                    .font(SlabFont.mono(size: 20, weight: .semibold))
                    .accessibilityIdentifier("manual-price-field")
                }
...
#Preview("New") {
    ManualPriceSheet(initialCents: nil) { _ in }
}

#Preview("Existing") {
    ManualPriceSheet(initialCents: 12_99) { _ in }
}
